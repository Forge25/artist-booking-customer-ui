      ...
      <div className="flex items-center flex-1 justify-between">
        {categories.map((cat, i) => (
          <button
            key={i}
            onClick={() => setActive(i)}
            className={`flex flex-col items-center gap-1.5 px-3 pb-1 transition-colors ${
              active === i ? 'text-red-500' : 'text-gray-400 hover:text-gray-600'
            }`}
          >
            {cat.icon}
            <span className={`text-xs font-medium ${active === i ? 'text-red-500' : 'text-gray-400'}`}>
              {cat.label}
            </span>
          </button>
        ))}
